...
func (app *appContext) NewUserPostVerification(p NewUserParams) (out NewUserData, pendingTasks *sync.WaitGroup) {
	...
	if app.usingLDAPIdentity() {
		exists, err := app.ldapIdentity.UserExists(p.Req.Username)
		if err != nil {
			out.Message = err.Error()
			deferLogError(lm.FailedCreateUser, "LDAP", p.Req.Username, out.Message)
			out.Status = 500
			return
		}
		if exists {
			out.Message = lm.UserExists
			deferLogInfo(lm.FailedCreateUser, "LDAP", p.Req.Username, out.Message)
			out.Status = 401
			return
		}

		var userID string
		userID, err = app.ldapIdentity.CreateUser(p.Req.Username, p.Req.Password)
		if err != nil {
			out.Message = err.Error()
			deferLogError(lm.FailedCreateUser, "LDAP", p.Req.Username, out.Message)
			out.Status = 401
	...
	if p.Profile != nil && !app.usingLDAPIdentity() {
...
func (app *appContext) SetUserDisabled(user mediabrowser.User, disabled bool) (err error, change bool, activityType ActivityType) {
	...
	if app.usingLDAPIdentity() && strings.HasPrefix(user.ID, LDAPUserIDPrefix) {
		err = app.ldapIdentity.SetDisabledByID(user.ID, disabled)
		if err != nil {
			return
		}
		change = true
...
func (app *appContext) DeleteUser(user mediabrowser.User) (err error, deleted bool) {
	if app.usingLDAPIdentity() && strings.HasPrefix(user.ID, LDAPUserIDPrefix) {
		err = app.ldapIdentity.DeleteUserByID(user.ID)
		if err != nil {
			return
		}
		deleted = true
