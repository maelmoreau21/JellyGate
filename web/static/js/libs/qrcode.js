/**
 * JellyGate QR Code Generator (Zero-dependency, 100% Client-Side & Offline)
...
(function (global) {
    ...
    Object.defineProperty(QR8bitByte.prototype, 'parsedData', {
        get: function () {
            ...
            for (var i = 0; i < this.data.length; i++) {
                ...
                if (charcode < 0x80) utf8.push(charcode);
                    ...
                    i++;
                    charcode = 0x10000 + (((charcode & 0x3ff) << 10) | (this.data.charCodeAt(i) & 0x3ff));
    ...
    var JGQRCode = {
        toSVG: function (text, options) {
            ...
            var darkColor = options.darkColor || '#09090b';
            var lightColor = options.lightColor || '#ffffff';
            var ecLevel = QRErrorCorrectLevel[options.ecLevel || 'M'] !== undefined ? QRErrorCorrectLevel[options.ecLevel || 'M'] : QRErrorCorrectLevel.M;
            ...
            var totalCount = count + margin * 2;
            var viewBox = '0 0 ' + totalCount + ' ' + totalCount;
            ...
            return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="' + viewBox + '" width="' + size + '" height="' + size + '" shape-rendering="crispEdges">' +
                (lightColor !== 'transparent' ? '<rect width="' + totalCount + '" height="' + totalCount + '" fill="' + lightColor + '" rx="1" />' : '') +
                rects.join('') +
                '</svg>';
        },

        toDataURL: function (text, options) {
            var svg = this.toSVG(text, options);
            return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
        },

        renderToImage: function (imgElement, text, options) {
            if (!imgElement) return;
            imgElement.src = this.toDataURL(text, options);
        }
    };

    global.JGQRCode = JGQRCode;
