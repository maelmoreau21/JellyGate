...
JFA_GO_CSS_VERSION="$(git describe --tags --abbrev=0 2>/dev/null || echo 'untagged')" JFA_GO_NFPM_EPOCH=$(git rev-list --all --count 2>/dev/null || echo 0) JFA_GO_BUILD_TIME=$(date +%s) JFA_GO_BUILT_BY=${JFA_GO_BUILT_BY:-"???"} JFA_GO_VERSION="$(echo $JFA_GO_VERSION | sed 's/v//g')" $@ --timeout $TIMEOUT
