...
`MatrixE2EE` builds (and Linux `TrayIcon` builds) include support for end-to-end encryption for the Matrix bot, but require the `libolm(-dev)` dependency. `.deb/.rpm/.apk` packages list this dependency, and docker images include it.
             ...
             ghcr.io/maelmoreau21/jellygate:latest
```

Identity backend can now be switched with:

- `JFA_USER_BACKEND=jellyfin` (default)
- `JFA_USER_BACKEND=ldap`

When using LDAP mode, configure these keys in `config.ini`:

- `[identity] backend = ldap`
- `[identity_ldap] server = ldaps://your-synology:636`
- `[identity_ldap] bind_dn = CN=svc_jfa,OU=Users,DC=example,DC=local`
- `[identity_ldap] bind_password = ...`
- `[identity_ldap] base_dn = DC=example,DC=local`
- `[identity_ldap] users_ou = OU=Users` (optional)
- `[identity_ldap] uid_attribute = uid` (or `sAMAccountName`)
- `[identity_ldap] upn_suffix = example.local` (optional)

A full compose template is available in `docker-compose.example.yml`.

A migration walkthrough for LDAP mode is available in `MIGRATION_LDAP.md`.
