...
// TemplateOld is the original implementation of the templater used in jfa-go.
// It does not support {if a = b}, only {if [!]a}, and will not fail when a braced block is incomplete (i.e. a stray "{" is left in the template).
func TemplateOld(content string, variables []string, conditionals []string, values map[string]interface{}) (string, error) {
	...
	for i, c := range content {
		if c == '{' {
			...
			ifEnd = i - 1
			if ifTrue {
				toAppend, subErr := TemplateOld(content[ifStart:ifEnd+1], variables, conditionals, values)
				out += toAppend
				if subErr != nil {
					err = subErr
				}
