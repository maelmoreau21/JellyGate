...
export class settingsList implements AsTab {
    ...
    private _showPanel = (name: string) => {
        ...
        if (button) {
            let parentGroup = button.inGroup();
            while (parentGroup) {
                const gb = this._groupButtons[parentGroup];
                if (!gb) break;
                gb.openCloseWithoutAnimation(true);
                parentGroup = gb.inGroup();
            }
            button.asElement().scrollIntoView({ block: "nearest", behavior: "smooth" });
        }
    };
