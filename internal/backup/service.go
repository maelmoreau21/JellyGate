...
const (
	...
	sqliteBackupEntry                   = "jellygate.db"
	postgresBackupEntry                 = "jellygate.dump"
	legacyPostgresSQLBackupEntry        = "jellygate.sql"
)
...
func (s *Service) CreateBackup(reason string) (BackupInfo, error) {
	...
	if s.db.IsSQLite() {
		...
		zipFileName = sqliteBackupEntry
	} else if s.db.IsPostgres() {
		dumpPath = filepath.Join(s.restoreDir, fmt.Sprintf("snapshot-%s.dump", stamp))
		defer os.Remove(dumpPath)
		...
		zipFileName = postgresBackupEntry
	} else {
		return info, fmt.Errorf("driver de base non supporte pour backup: %q", s.db.Driver())
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600) // #nosec G304 -- path is generated by BackupPath inside the configured backup directory.
	if err != nil {
	...
	dbBytes, err := os.ReadFile(dumpPath) // #nosec G304 -- dumpPath is generated internally in the restore work directory.
	if err != nil {
		_ = zw.Close()
...
func extractBackupEntry(archivePath, entryName, outputPath string) error {
	...
	for _, file := range zr.File {
		...
		out, err := os.OpenFile(outputPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600) // #nosec G304 -- outputPath is generated internally by the restore workflow.
		if err != nil {
...
func (s *Service) RestorePostgresBackup(name string) error {
	...
	dumpPath := filepath.Join(s.restoreDir, fmt.Sprintf("restore-%s.dump", stamp))
	defer os.Remove(dumpPath)

	if err := extractBackupEntry(archivePath, postgresBackupEntry, dumpPath); err != nil {
		return err
	}
...
func (s *Service) ImportBackup(filename string, r io.Reader) (BackupInfo, error) {
	...
	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600) // #nosec G304 -- imported backup name is generated server-side under backupDir.
	if err != nil {
	...
	requiredEntry := sqliteBackupEntry
	if s.db != nil && s.db.IsPostgres() {
		requiredEntry = postgresBackupEntry
	}
...
func validateBackupArchive(path string) error {
	...
	hasLegacyPostgresSQLDump := false
	...
	for _, f := range zr.File {
		...
		if base == sqliteBackupEntry {
			hasSQLiteDump = true
		}
		if base == postgresBackupEntry {
			hasPostgresDump = true
		}
		if base == legacyPostgresSQLBackupEntry {
			hasLegacyPostgresSQLDump = true
		}
	}
	if hasLegacyPostgresSQLDump {
		return fmt.Errorf("archive invalide: les sauvegardes PostgreSQL plain SQL ne sont plus restaurables")
	}
	if !hasSQLiteDump && !hasPostgresDump {
		return fmt.Errorf("archive invalide: fichier %s ou %s manquant", sqliteBackupEntry, postgresBackupEntry)
	}
	return nil
}
...
func (s *Service) PrepareRestore(name string) error {
	...
	if err := extractBackupEntry(archivePath, sqliteBackupEntry, pendingDB); err != nil {
		return err
	}
...
func copyFile(src, dst string) error {
	in, err := os.Open(src) // #nosec G304 -- source path is generated by backup restore/apply code.
	if err != nil {
		return err
	}
	defer in.Close()

	out, err := os.OpenFile(dst, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600) // #nosec G304 -- destination path is generated by backup restore/apply code.
	if err != nil {
		return err
	}
	defer out.Close()
