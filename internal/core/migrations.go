...
// Migrate poorly-named and duplicate "url_base" settings to the single "external jellygate URL" setting.
func migrateExternalURL(app *appContext) {
	...
	fmt.Println(warning("The duplicate URL Base settings in \"Invite emails\" and \"Password Resets\" have been merged into General > External jellygate URL. A backup config has been made."))
...
func migratePWREmailPath(app *appContext) {
	if app.config.Section("password_resets").Key("email_html").String() != "jellygate:email.html" && app.config.Section("password_resets").Key("email_text").String() != "jellygate:email.txt" {
		return
	}
	tempConfig, _ := ini.ShadowLoad(app.configPath)
	if app.config.Section("password_resets").Key("email_html").String() == "jellygate:email.html" {
		tempConfig.Section("password_resets").Key("email_html").SetValue("")
	}
	if app.config.Section("password_resets").Key("email_text").String() == "jellygate:email.txt" {
