...
type appContext struct {
	...
	userCache            *UserCache
	identityBackendMode  string
	ldapIdentity         *LDAPIdentityProvider
}

func parseEnvInt(keys ...string) (int, bool) {
...
func (app *appContext) applyProcessIdentityFromEnv() {
...
func start(asDaemon, firstCall bool) {
	...
	/*
		...
		localFS: jellygate's internal data. On internal builds, this is contained within the binary.
				 On external builds, the directory is named "data" and placed next to the executable.
	*/
	userConfigDir, _ := os.UserConfigDir()
	app.dataPath = filepath.Join(userConfigDir, "jellygate")
	...
	if v := strings.TrimSpace(os.Getenv("JFA_JELLYFIN_SERVER")); v != "" {
		app.config.Section("jellyfin").Key("server").SetValue(v)
	}
	if v := strings.TrimSpace(os.Getenv("JFA_JELLYFIN_PUBLIC_SERVER")); v != "" {
		app.config.Section("jellyfin").Key("public_server").SetValue(v)
	}
	if v := strings.TrimSpace(os.Getenv("JFA_JELLYFIN_API_KEY")); v != "" {
		app.config.Section("jellyfin").Key("api_key").SetValue(v)
	}
	if err := app.configureIdentityBackend(); err != nil {
		app.err.Fatalf("failed to configure identity backend: %v", err)
	}
	...
	// Starts listener to receive commands over a unix socket. Use with 'jellygate start/stop'
	if asDaemon {
		go func() {
			os.Remove(SOCK)
	...
	if !firstRun {
		...
		if port, ok := parseEnvInt("PORT", "port", "JFA_PORT"); ok && port > 0 {
			app.port = port
		}
		...
		apiKey := strings.TrimSpace(app.config.Section("jellyfin").Key("api_key").String())
		if apiKey != "" {
			app.jf.AccessToken = apiKey
			app.jf.Authenticated = true
			_, err = app.jf.GetUsers(false)
			if err != nil {
				app.err.Fatalf(lm.FailedAuthJellyfin, server, status, err)
			}
			app.info.Printf(lm.AuthJellyfin, server)
			app.debug.Printf(lm.AsUser, "api_key")
		} else {
		...
		if jfLogin, _ := app.config.Section("ui").Key("jellyfin_login").Bool(); !jfLogin {
			...
			app.authJf, _ = mediabrowser.NewServer(serverType, server, "jellygate", app.version, "auth", "auth", timeoutHandler, cacheTimeout)
			if debugMode {
		...
		// Test mode for testing connection to Jellyfin, accessed with 'jellygate test'
		if TEST {
			test(app)
			os.Exit(0)
		}
...
// @title jellygate internal API
// @version 0.6.1
// @description API for the jellygate frontend
// @contact.name Harvey Tindall
// @contact.email hrfee@hrfee.dev
// @license.name MIT
// @license.url https://raw.githubusercontent.com/hrfee/jellygate/main/LICENSE
// @BasePath /
...
// @tag.description jellygate settings.
...
func printVersion() {
	...
	fmt.Println(info("JellyGate version: %s (%s)%s\n", hiwhite(version), white(commit), tray))
}

const SYSTEMD_SERVICE = "jellygate.service"
...
func main() {
	...
	if flagPassed("start") {
		...
		fmt.Println(info(`If you want to execute jellygate with special arguments, re-run this command with them.
...
`))
		...
		time.Sleep(time.Millisecond)
		fmt.Print(info("systemctl --user start jellygate\n\n"))
		time.Sleep(time.Millisecond)
		color.New(color.FgRed).Print("To stop: ")
		time.Sleep(time.Millisecond)
		fmt.Print(info("systemctl --user stop jellygate\n\n"))
		time.Sleep(time.Millisecond)
