...
func (app *appContext) EnableDisableUsers(gc *gin.Context) {
	...
	for _, userID := range req.Users {
		user, err := app.identityUserByID(userID)
		if err != nil {
			errors["GetUser"][userID] = err.Error()
...
func (app *appContext) getIdentityUserSummaries() ([]*respUser, error) {
	if app.usingLDAPIdentity() {
		users, err := app.ldapIdentity.ListUsers()
		if err != nil {
			return nil, err
		}
		out := make([]*respUser, 0, len(users))
		for _, user := range users {
			summary := app.GetUserSummary(user)
			out = append(out, &summary)
		}
		return out, nil
	}
	return app.userCache.GetUserDTOs(app, true)
}

func (app *appContext) getIdentityUserIDs() ([]string, error) {
	if app.usingLDAPIdentity() {
		users, err := app.ldapIdentity.ListUsers()
		if err != nil {
			return nil, err
...
func (app *appContext) getIdentityUserIDSet() (map[string]struct{}, error) {
	ids, err := app.getIdentityUserIDs()
	if err != nil {
		return nil, err
	}
...
func (app *appContext) GetUserCount(gc *gin.Context) {
	...
	if err != nil {
		...
		app.err.Printf(lm.FailedGetUsers, service, err)
		respond(500, "Couldn't get users", gc)
		return
	}
	resp.Count = uint64(len(ids))
	gc.JSON(200, resp)
}
...
func (app *appContext) GetLabels(gc *gin.Context) {
	if app.usingLDAPIdentity() {
		userList, err := app.getIdentityUserSummaries()
		if err != nil {
			app.err.Printf(lm.FailedGetUsers, "LDAP", err)
			respond(500, "Couldn't get users", gc)
			return
		}
		...
		slices.Sort(labels)
		gc.JSON(200, LabelsDTO{Labels: labels})
		return
	}
...
func (app *appContext) GetUser(gc *gin.Context) {
	...
	if err != nil {
		if app.usingLDAPIdentity() && strings.HasPrefix(userID, LDAPUserIDPrefix) {
			respondBool(400, false, gc)
			app.err.Printf(lm.FailedGetUser, userID, "LDAP", err)
			return
		}
		switch err.(type) {
...
func (app *appContext) GetUsers(gc *gin.Context) {
	...
	resp.LastPage = true
	var err error
	resp.UserList, err = app.getIdentityUserSummaries()
	if err != nil {
		service := lm.Jellyfin
		if app.usingLDAPIdentity() {
			service = "LDAP"
...
// @Summary Set whether or not a user can access jellygate. Redundant if the user is a Jellyfin admin.
// @Produce json
...
func (app *appContext) SetAccountsAdmin(gc *gin.Context) {
	...
	gc.BindJSON(&req)
	userIDSet, err := app.getIdentityUserIDSet()
	if err != nil {
		service := lm.Jellyfin
		if app.usingLDAPIdentity() {
			service = "LDAP"
...
// @Summary Gets the number of Jellyfin/Emby activities stored by jellygate related to the given user ID. As the total collected by jellygate is limited, this may not include all those held by Jellyfin.
// @Produce json
// @Success 200 {object} PageCountDTO
// @Failure 400 {object} boolResponse
...
// @Summary Get the latest Jellyfin/Emby activities related to the given user ID. Returns as many as the server has recorded. As the total collected by jellygate is limited, this may not include all those held by Jellyfin.
// @Produce json
...
// @Summary Get the latest Jellyfin/Emby activities related to the given user ID, paginated. As the total collected by jellygate is limited, this may not include all those held by Jellyfin.
// @Produce json
