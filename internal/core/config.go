...
func (config *Config) GetPath(sect, key string) (fs.FS, string) {
	...
	if strings.HasPrefix(val, "jellygate:") {
		return localFS, strings.TrimPrefix(val, "jellygate:")
	}
...
// ExternalURI returns the External URI of jellygate's root directory (by default, where the admin page is), using the fixed externalURI value unless UseProxyHost is true and gc is not nil.
...
func NewConfig(configPathOrContents any, dataPath string, logs LoggerSet) (*Config, error) {
	...
	config.MustSetValue("jellyfin", "api_key", "")
	config.MustSetValue("identity", "backend", "jellyfin")
	config.MustSetValue("identity_ldap", "server", "")
	config.MustSetValue("identity_ldap", "bind_dn", "")
	config.MustSetValue("identity_ldap", "bind_password", "")
	config.MustSetValue("identity_ldap", "base_dn", "")
	config.MustSetValue("identity_ldap", "users_ou", "")
	config.MustSetValue("identity_ldap", "uid_attribute", "uid")
	config.MustSetValue("identity_ldap", "sam_attribute", "sAMAccountName")
	config.MustSetValue("identity_ldap", "upn_suffix", "")
	config.MustSetValue("identity_ldap", "object_classes", "top|person|organizationalPerson|user")
	config.MustSetValue("identity_ldap", "disabled_attribute", "userAccountControl")
	config.MustSetValue("identity_ldap", "disabled_value", "true")
	config.MustSetValue("identity_ldap", "enabled_value", "false")
	config.MustSetValue("identity_ldap", "insecure_skip_verify", "false")
	...
	// config.MustSetValue("password_resets", "email_html", "jellygate:"+"password-reset.html")
	// config.MustSetValue("password_resets", "email_text", "jellygate:"+"password-reset.txt")

	// config.MustSetValue("invite_emails", "email_html", "jellygate:"+"invite-email.html")
	// config.MustSetValue("invite_emails", "email_text", "jellygate:"+"invite-email.txt")

	// config.MustSetValue("email_confirmation", "email_html", "jellygate:"+"confirmation.html")
	// config.MustSetValue("email_confirmation", "email_text", "jellygate:"+"confirmation.txt")

	// config.MustSetValue("notifications", "expiry_html", "jellygate:"+"expired.html")
	// config.MustSetValue("notifications", "expiry_text", "jellygate:"+"expired.txt")

	// config.MustSetValue("notifications", "created_html", "jellygate:"+"created.html")
	// config.MustSetValue("notifications", "created_text", "jellygate:"+"created.txt")

	// config.MustSetValue("deletion", "email_html", "jellygate:"+"deleted.html")
	// config.MustSetValue("deletion", "email_text", "jellygate:"+"deleted.txt")

	// Deletion template is good enough for these as well.
	// config.MustSetValue("disable_enable", "disabled_html", "jellygate:"+"deleted.html")
	// config.MustSetValue("disable_enable", "disabled_text", "jellygate:"+"deleted.txt")
	// config.MustSetValue("disable_enable", "enabled_html", "jellygate:"+"deleted.html")
	// config.MustSetValue("disable_enable", "enabled_text", "jellygate:"+"deleted.txt")

	// config.MustSetValue("welcome_email", "email_html", "jellygate:"+"welcome.html")
	// config.MustSetValue("welcome_email", "email_text", "jellygate:"+"welcome.txt")

	// config.MustSetValue("template_email", "email_html", "jellygate:"+"template.html")
	// config.MustSetValue("template_email", "email_text", "jellygate:"+"template.txt")

	config.MustSetValue("user_expiry", "behaviour", "disable_user")
	// config.MustSetValue("user_expiry", "email_html", "jellygate:"+"user-expired.html")
	// config.MustSetValue("user_expiry", "email_text", "jellygate:"+"user-expired.txt")

	// config.MustSetValue("user_expiry", "adjustment_email_html", "jellygate:"+"expiry-adjusted.html")
	// config.MustSetValue("user_expiry", "adjustment_email_text", "jellygate:"+"expiry-adjusted.txt")

	// config.MustSetValue("user_expiry", "reminder_email_html", "jellygate:"+"expiry-reminder.html")
	// config.MustSetValue("user_expiry", "reminder_email_text", "jellygate:"+"expiry-reminder.txt")
	...
	for _, cc := range customContent {
		...
		for i := range fnameSettingSuffix {
			config.MustSetValue(cc.SourceFile.Section, cc.SourceFile.SettingPrefix+fnameSettingSuffix[i], "jellygate:"+cc.SourceFile.DefaultValue+"."+fnameExtension[i])
		}
	}
	...
	config.Section("jellyfin").Key("device").SetValue("jellygate")
	config.Section("jellyfin").Key("device_id").SetValue(fmt.Sprintf("jellygate-%s-%s", version, commit))
