...
func (app *appContext) ResetSetPassword(gc *gin.Context) {
	...
	if reset, ok := app.internalPWRs[req.PIN]; ok {
		...
		if app.usingLDAPIdentity() {
			app.err.Printf(lm.FailedChangePassword, "LDAP", "?", "external jellyfin reset PINs are unsupported in LDAP mode")
	...
	if app.usingLDAPIdentity() {
		if username == "" {
			app.err.Printf(lm.FailedChangePassword, "LDAP", userID, "missing username")
			respondBool(500, false, gc)
			return
		}
		if err := app.ldapIdentity.SetPasswordByUsername(username, req.Password); err != nil {
			app.err.Printf(lm.FailedChangePassword, "LDAP", username, err)
			respondBool(500, false, gc)
			return
		}
		id := app.ldapIdentity.userID(username)
...
// @Summary Get jellygate configuration.
// @Produce json
